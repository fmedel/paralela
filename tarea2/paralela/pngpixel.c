 ...
 */
static void
print_pixel(png_structp png_ptr, png_infop info_ptr, png_const_bytep row, png_uint_32 x,char *nombre)
{
   PNG_CONST unsigned int bit_depth = png_get_bit_depth(png_ptr, info_ptr);
   FILE *fp;
    fp = fopen ( nombre, "a" );/* imprimir  los pixcel */
    int c1a;
    int c2a;
    int c3a;
...
int funcion_main(int argc,long  fila, long  columna, char * nombre, int seccion_fila, int seccion_columna, char *nombre_2)
{
               ...
               if (setjmp(png_jmpbuf(png_ptr)) == 0)
               {
                     ...
                     &compression_method, &filter_method))
                  {
                     ...
                     for (pass=0; pass<passes; ++pass)
                     {
                        ...
                        for (py = 0; py < height; py += ystep)
                        //for (py = ystart; py < (800*seccion_fila); py += ystep)
                        {
                           png_uint_32 px, ppx;
                            ...
                            */
                             ...
                             //{
                                 ...
                                 * buffer.
                                 */
                                print_pixel(png_ptr, info_ptr, row_tmp, ppx,nombre_2);
